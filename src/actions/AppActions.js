...
export const adicionaContato = email => (
    // o redux thunk é utilizado pra deixar  a action síncrona
    dispatch => {
        const emailB64 = b64.encode(email);
        // a função on recupera em tempo real
        // a função once faz apenas uma unica checagem quando é chamado
        ...
        firebase.database().ref(`/mensagens/${usuarioEmailB64}/${contatoEmailB64}`)
            ...
            .then(() => {
                firebase.database().ref(`/mensagens/${contatoEmailB64}/${usuarioEmailB64}`)
                    ...
                    .then(() => dispatch({ type: LIMPA_CAMPO_TEXTO }));
            })
            .then(() => { //armazenar o cabeçalho de conversa do usuario
                firebase.database().ref(`/usuario_conversas/${usuarioEmailB64}/${contatoEmailB64}`)
                    .set({ nome: contatoNome, email: contatoEmail });      
            })
            .then(() => { // armazenar o cabeçalho de conversa do contato
                // Pegando o nome do usuario autenticado
